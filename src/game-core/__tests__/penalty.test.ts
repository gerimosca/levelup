...
describe('applyDailyDecay — red de seguridad (NUNCA bajar de nivel)', () => {
  it('aplica la penalización completa cuando hay margen sobre el suelo', () => {
    ...
    const result = applyDailyDecay(xp, [HABITS.train, HABITS.no_alcohol]); // 30 + 38
    expect(result.rawPenalty).toBe(68);
    expect(result.newXpTotal).toBe(1932);
    expect(result.xpLost).toBe(68);
